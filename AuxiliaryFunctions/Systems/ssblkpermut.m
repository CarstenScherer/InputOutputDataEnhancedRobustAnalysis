...
%  
function [G] = ssblkpermut(G, bdim, bper)
...
% Some sanity checks
if la ~= ls
    error('Number of states and partitions do not match')
elseif length(bdim) ~= length(bper)
    error(['Length of state permutation and block partition', ...
           'do not match'])
elseif min(bper) <= 0 || max(bper) > length(bdim)
    error('State permutation is not a valid permutation')
